<x-app-layout>
    <x-slot name="header">
        ...
        <div class="grid grid-cols-12 gap-2 mb-2 mt-2">
            ...
            <div class="col-span-12 lg:col-span-5 flex items-end justify-start space-x-2">
                @can('tickets.update')
                    <a href="{{ route('tickets.edit', $ticket->id) }}" class="inline-flex items-center justify-center px-4 py-2 bg-blue-400 hover:bg-blue-500 text-white text-sm font-medium rounded shadow-sm transition duration-150 ease-in-out">
                        <i class="fas fa-edit mr-2"></i>Editar Ticket
                    </a>
                @endcan
                @can('tickets.destroy')
                    <button type="button" class="eliminar inline-flex items-center justify-center px-4 py-2 bg-red-500 hover:bg-red-600 text-white text-sm font-medium rounded shadow-sm transition duration-150 ease-in-out" data-form="eliminar-ticket-{{ $ticket->id }}" data-model="Ticket">
    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 md:p-6 mb-4">
        <div class="flex flex-row flex-wrap">
            <!-- Título -->
            @if($ticket->titulo)
                <div class="basis-full p-2 md:p-4">
            ...
            <!-- Cliente -->
            <div class="basis-full md:basis-1/2 lg:basis-1/3 p-2 md:p-4">
                <x-label class="text-blue-900 font-semibold mb-1">
                ...
                <div class="basis-full p-2 md:p-4">
                    ...
                    <div class="flex flex-wrap gap-2 mt-2">
                        @foreach ($ticket->tags as $tag)
                            <span class="px-4 py-2 rounded-full text-gray-700 bg-gray-200 font-semibold text-sm hover:bg-sky-100 transition duration-300">
                                {{ $tag->nombre }}
                            </span>
                        @endforeach
                    </div>
            ...
            @if($ticket->respuestas->count() > 0)
                @foreach ($ticket->respuestas as $respuesta)
                    <div class="flex {{ $respuesta->user->cliente ? 'flex-row' : 'flex-row-reverse' }} space-x-2">
                        <div class="rounded-xl m-1 p-3 basis-full md:basis-7/12 {{ $respuesta->user->cliente ? 'bg-cyan-300' : 'bg-green-200' }}">
        ...
        <div class="bg-white rounded-lg shadow-sm border border-gray-200 p-3 md:p-6">
            ...
            <form action="{{ route('respuestas.store') }}" name="dropzone-form" method="post" class="drozone"
                    ...
                    <div id="predetermined-messages" class="mt-2 mb-2 w-full">
                        @if ($mensajesFrecuentes->isNotEmpty())
                            @foreach ($mensajesFrecuentes as $mensaje)
                                <button
                                    class="message-btn px-4 py-2 bg-blue-600 text-sm text-white rounded-md hover:bg-blue-700 mr-2 mb-2"
                        ...
                        @else
                            <button
                                class="message-btn px-4 py-2 mt-2 bg-blue-600 text-sm text-white rounded-md hover:bg-blue-700 mr-2"
                ...
                <div class="basis-full mt-2">
                    ...
                    <button type='submit' id="enviar" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-medium rounded shadow-sm transition">
                        Enviar &nbsp;&nbsp;
                    ...
                    <button type='button' id="cerrar_ticket"
                        class="ml-3 inline-flex items-center px-4 py-2 bg-green-600 hover:bg-green-700 text-white text-sm font-medium rounded shadow-sm transition">
                        Cerrar &nbsp;&nbsp;
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-3 w-3" fill="none"
                            viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                            <path stroke-linecap="round" stroke-linejoin="round"
