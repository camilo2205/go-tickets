<x-app-layout>
        ...
        <div class="flex flex-row content-end">
            <div class="basis-1/3">
                <span class="text-sm text-gray-800">
                    <a class="underline" href="{{ route('tareas.index') }}">Tareas</a> /
                    <a class="underline" href="{{ route('tareas.show', $tarea->id) }}">{{ $tarea->nombre }}</a> /
                    <span class="font-bold">Editar</span>
                </span>
            </div>
            <div class="basis-1/3 self-end">
                <h2 class="font-semibold text-2xl text-gray-800 leading-tight">{{ __('EDITAR TAREA') }}</h2>
            </div>
            @error('error')
            <div class="basis-1/3">
                <x-small-message class="bg-red-200 text-red-600 w-fit p-1 rounded font-bold">{{ $message }}
                </x-small-message>
            </div>
            @enderror
        </div>
    ...
    <form action="{{ route('tareas.update', $tarea->id) }}" method="post" class="flex flex-row flex-wrap space-y-4">
        @csrf
        @method('PUT')
        <!-- Guardar -->
        ...
        <div class="basis-full px-2">
            ...
            <x-input id="nombre" class="block mt-1 w-full" type="text" name="nombre" :value="old('nombre', $tarea->nombre)"
                autocomplete="off" />
            @error('nombre')
        ...
        <div class="md:basis-1/3 px-2">
            ...
            <x-select name="encargado_id" id="encargado_id" class="select2" autocomplete="name">
                ...
                <option value="{{ $encargado->id }}" {{ old('encargado_id', $tarea->encargado_id)==$encargado->id ? 'selected' : '' }}>{{
        ...
        <div class="basis-full px-2">
            ...
            <x-textarea id="descripcion" class="block mt-1 w-full" name="descripcion"
                placeholder="Descripción de la tarea" autocomplete="off">{{ old('descripcion', $tarea->descripcion) }}</x-textarea>
            @error('descripcion')
            <x-small>{{ $message }}</x-small>
            @enderror
        </div>
